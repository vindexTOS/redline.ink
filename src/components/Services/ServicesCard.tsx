...
const ServicesCard: FC<CardPropTypes> = ({ icon, title, description, link, setOpenSingleService }) => {
	return (
		<div className="  boxShaddow  p-3 flex flex-col items-center justify-around w-[400px] h-[500px] bg-black/40 rounded-lg">
			...
			<div className="flex  flex-col    justify-around gap-3">
				<h1 className="text-xl font-bold text-gray-400 mb-2 text-center">{title}</h1>

				<p className="text-gray-200 mb-1  text-center">{description}</p>

				<button
					onClick={() => setOpenSingleService(link)}
